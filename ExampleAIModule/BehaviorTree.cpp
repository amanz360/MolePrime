...
public:
	...
	{  // RandomSelector operates as a Selector, but in a random order instead of from first child to last child (adds more unpredictability to the behaviour when a there is no clear preferred order of execution).
		public:
			virtual bool run() override 
			{
	...
	{  // Function is either to transform the result it receives from its child node's status, to terminate the child, or repeat processing of the child, depending on the type of decorator node.
		private:
	...
	{  // Inverts the result of the child. A child fails and it will return success to its parent, or a child succeeds and it will return failure to the parent.
		private:
			virtual bool run() override { return !getChild()->run(); }
	};
